...
PREBUILT_DIR="${HOME}/arm"
IREE_ARTIFACT_URL="https://sharkpublic.blob.core.windows.net/sharkpublic/GCP-Migration-Files"

read -p "Enter the arm tools root path(press enter to use default path:${PREBUILT_DIR}): " INPUT_PATH
if [[ "${INPUT_PATH}" ]]; then
  PREBUILT_DIR=${INPUT_PATH}
fi
echo "The arm tool prefix path: ${PREBUILT_DIR}"

BOOTSTRAP_WORK_DIR="${PREBUILT_DIR}/.bootstrap"

if [[ "${OSTYPE}" == "linux-gnu" ]]; then
  ARM_CLANG_TOOLCHAIN_FILE_NAME="toolchain_iree_manylinux_2_28_20231012.tar.gz"
  ARM_CLANG_TOOLCHAIN_FILE_SHA="3af56a58551ed5ae7441214822461a5368fee9403d7c883762fa902489bfbff0"
  ...
  TOOLCHAIN_PATH_PREFIX=${PREBUILT_DIR}/toolchain/clang/linux/ARM
  QEMU_PATH_PREFIX=${PREBUILT_DIR}/qemu/linux/ARM
else
...
read -p "Install ARM clang toolchain(y/n)? " answer
case ${answer:0:1} in
  y|Y )
    download_file "${ARM_CLANG_TOOLCHAIN_FILE_NAME}" \
                  "${TOOLCHAIN_PATH_PREFIX}" \
                  "${ARM_CLANG_TOOLCHAIN_FILE_SHA}"

    echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    echo " PLEASE run 'export ARM_TOOLCHAIN_ROOT=${TOOLCHAIN_PATH_PREFIX}'   "
    echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
  ;;
  * )
    echo "Skip ARM clang toolchain."
  ;;
esac

read -p "Install ARM qemu(y/n)? " answer
case ${answer:0:1} in
  y|Y )
